package com.spotfinder.backend.v1.iam.infrastructure.tokens.jwt.services;
...
public class TokenServiceImpl implements BearerTokenService {
    ...
    private static final String CLAIM_EMAIL_VERIFIED = "isEmailVerified";
    ...
    /**
     * Generates a JWT token using explicit subject (userId) and claims.
     */
    @Override
    public String generateToken(Long userId, String email, java.util.List<String> roles, boolean isEmailVerified) {
        ...
        var builder = Jwts.builder()
                ...
                .claim(CLAIM_EMAIL_VERIFIED, isEmailVerified)
                .signWith(key);
    ...
    public String getUsernameFromToken(String token) {
        // Return email claim for compatibility with authentication pipeline
    ...
    public String getUserIdFromToken(String token) {
        // Primary: subject should be the numeric user id
        String sub = extractClaim(token, Claims::getSubject);
        if (isNumeric(sub)) return sub;
        // Fallback for legacy tokens where subject=email
        try {
            String email = getEmailFromToken(token);
            if (email != null) {
                var user = userRepository.findByEmail(email);
                if (user.isPresent()) return String.valueOf(user.get().getId());
            }
        } catch (Exception ignored) {}
        return sub; // last resort
    }
